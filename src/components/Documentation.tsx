...
const SECTIONS = [
  ...
  { id: 'arsitektur', label: 'Arsitektur', icon: GitCommit, title: 'Cara Kerja KNN' },
  { id: 'preprocessing', label: 'Prapemrosesan', icon: Layers, title: 'Tahap Prapemrosesan' },
  { id: 'performa', label: 'Performa', icon: ShieldCheck, title: 'Hasil Evaluasi' },
  { id: 'referensi', label: 'Referensi', icon: Database, title: 'Fitur & Variabel' },
  ...
  return (
    <div className="space-y-6 text-slate-600 font-light leading-relaxed">
      <p className="text-base sm:text-lg">
        HIV masih jadi masalah kesehatan yang serius di Indonesia. Setiap tahun ribuan orang terdiagnosis, dan proses skriningnya masih banyak yang manual. Nah, di sinilah teknologi bisa bantu — khususnya machine learning.
      </p>
      ...
      <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-900">
        <strong>Catatan:</strong> Sistem ini cuma untuk edukasi dan tugas kuliah. Bukan pengganti diagnosis dokter. Kalau butuh diagnosis akurat, tetap harus ke fasyankes dan uji lab kayak ELISA atau Western Blot.
      </div>
    </div>
  );
}
...
function ArsitekturContent() {
  return (
    <div className="space-y-6 text-slate-600 font-light leading-relaxed">
      <p className="text-base sm:text-lg">
        KNN itu termasuk <em>supervised learning</em> — artinya kita kasih contoh data yang sudah punya label, terus algoritma belajar dari contoh itu. Waktu ada data baru, dia tinggal cari siapa yang paling mirip di data latih.
      </p>
      ...
      <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-sm">
        ...
        <ul className="space-y-6 m-0 p-0 text-sm sm:text-base list-none">
          {[
            { title: '1. Hitung Jarak', desc: 'Setiap data baru diukur jaraknya ke SEMUA data latih pakai Euclidean Distance. Rumusnya: √Σ(x₁ - x₂)². Makin kecil jaraknya, makin mirip.' },
            { title: '2. Urutkan Jarak', desc: 'Jarak dari semua data latih diurutkan dari terkecil ke terbesar. Kita ambil K terdekat (default K=3).' },
            { title: '3. Voting Mayoritas', desc: 'Dari K tetangga terdekat, lihat label mana yang paling banyak muncul. Itu yang jadi prediksi. Misal: 2 tetangga bilang ODHIV, 1 bilang Bukan → hasilnya ODHIV.' }
          ].map((item, idx) => (
      ...
      <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl">
        <h4 className="text-sm font-semibold text-slate-900 mb-2">Kenapa perlu Cross-Validation?</h4>
        <p className="text-sm text-slate-600">
          Nilai K itu penting. Kalau K terlalu kecil (misal 1), model gampang kena noise. Kalau K terlalu besar, justru data yang sebenernya beda kelas jadi keangkut. Makanya kita pakai 5-Fold Cross-Validation buat cari K paling optimal — diuji dari K=1 sampai K=15, yang paling bagus F1-Score-nya, itulah yang dipakai.
        </p>
      </div>

      <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl">
        <h4 className="text-sm font-semibold text-slate-900 mb-2">Normalisasi: Kenapa Penting?</h4>
        <p className="text-sm text-slate-600">
          Bayangin kalau ada fitur "Usia" (21-62) dan "Jenis Kelamin" (0-1). Tanpa normalisasi, Usia bakal mendominasi perhitungan jarak karena angkanya jauh lebih gede. Solusinya? Pakai Min-Max Normalization biar semua fitur punya range yang sama (0 sampai 1).
        </p>
      </div>
    </div>
  );
...
function PreprocessingContent() {
  return (
    <div className="space-y-6 text-slate-600 leading-relaxed font-light">
      ...
      <p className="text-base sm:text-lg">
        Ada <strong>5 tahap preprocessing</strong> yang harus dijalankan sebelum data masuk ke KNN. Setiap tahap penting dan punya alasan tersendiri:
      </p>
      ...
      <div className="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <p className="text-xs text-slate-500 italic mt-2">
          Kenapa harus dipisah? Kalau data testing dipake saat training, model cuma "menghafal" jawabannya. Hasilnya kelihatan bagus tapi sebenernya nggak akurat — ini yang disebut data leakage. Seed=42 dipake biar tiap kali dijalankan, hasilnya sama.
        </p>
      </div>
      ...
      <div className="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h5 className="font-bold text-slate-900 text-lg">SMOTE (Seimbangkan Kelas)</h5>
        </div>
        <p className="text-slate-600 font-light text-sm leading-relaxed mb-3">
          Di data kita, kelas "Belum Tahu" cuma punya 1-2 data. Kalau dibiarkan, KNN bakal lebih sering nebak kelas mayoritas karena jumlahnya lebih banyak. SMOTE bikin data sintetis buat kelas yang kurang — tapi bukan duplikat, melainkan interpolasi antara data yang ada.
        </p>
        <div className="p-3 bg-slate-50 rounded-lg text-xs text-slate-600 space-y-1">
          <p><strong>Input:</strong> Data training yang kelasnya timpang</p>
          <p><strong>Proses:</strong> Untuk kelas minoritas → cari tetangga → bikin data baru di antara mereka</p>
          <p><strong>Output:</strong> Data training seimbang, semua kelas jumlahnya sama</p>
        </div>
        <p className="text-xs text-slate-500 italic mt-2">
          Rumus SMOTE: x_sintetis = x_i + random(0,1) × (x_nn - x_i). Artinya, data baru ditaruh di jalur antara data asli dan tetangganya. Sesuai jurnal: "SMOTETomek sebagai metode resampling untuk mengatasi ketidakseimbangan kelas."
        </p>
      </div>
      ...
      <div className="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <p className="text-slate-600 font-light text-sm leading-relaxed mb-3">
          Tahap terakhir. Semua fitur dikompres ke range 0-1 supaya punya bobot yang adil. Rumusnya: (nilai - min) / (max - min). Kalau nilai = min → hasilnya 0. Kalau nilai = max → hasilnya 1.
        </p>
        <div className="p-3 bg-slate-50 rounded-lg text-xs text-slate-600 space-y-1">
          <p><strong>Input:</strong> Data training seimbang dengan nilai asli</p>
          <p><strong>Proses:</strong> Hitung min/max tiap fitur → terapkan rumus ke semua data</p>
...
function PerformaContent() {
  ...
  if (!metrics) {
    return <p className="text-sm text-slate-500">Gagal memuat metrik performa.</p>;
  }

  return (
    <div className="space-y-6">
      <p className="text-sm sm:text-base text-slate-600 leading-relaxed">
        Berikut hasil evaluasi model KNN dengan K=3 dan 5-Fold Cross-Validation. Angka ini dihitung langsung dari data, bukan angka asal.
      </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-5">
        {[
          ...
          <div key={idx} className={`p-6 rounded-2xl flex flex-col items-center justify-center text-center transition-all hover:-translate-y-1 ${
            ...
            <div className={`text-3xl sm:text-4xl font-extrabold mb-2 tracking-tight ${idx === 0 ? 'text-white' : 'text-slate-900'}`}>{(stat.value * 100).toFixed(1)}%</div>
            <div className={`text-[10px] sm:text-xs uppercase tracking-widest font-bold ${idx === 0 ? 'text-slate-400' : 'text-slate-500'}`}>{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-emerald-50/50 p-6 sm:p-8 rounded-2xl border border-emerald-100 relative overflow-hidden flex gap-4 items-start sm:items-center">
        ...
        <div className="text-sm sm:text-base text-slate-700 leading-relaxed space-y-2">
          <p>
            Recall di angka <strong>{(metrics.macroRecall * 100).toFixed(1)}%</strong> artinya dari semua pasien yang memang ODHIV, hampir semuanya berhasil dikenali oleh sistem. Ini penting banget — karena kalau ada ODHIV yang nggak terdeteksi (false negative), dampaknya fatal.
          </p>
          <p>
            Precision di <strong>{(metrics.macroPrecision * 100).toFixed(1)}%</strong> berarti dari yang diprediksi positif, mayoritas memang benar positif. Jadi false alarm-nya rendah.
          </p>
        </div>
      </div>
    ...
    <div className="space-y-6 sm:space-y-8 w-full">
      <div className="bg-indigo-50 text-indigo-900 p-5 sm:p-6 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
        ...
        <p className="text-sm sm:text-base leading-relaxed font-light m-0">
          Semua kode di sini berjalan di browser (client-side). Nggak ada server, nggak ada data yang dikirim ke mana-mana. Prediksi langsung di laptop/HP kamu. Privasi aman.
        </p>
      </div>
      ...
      <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm">
        <div className="bg-slate-50 px-4 sm:px-6 py-3.5 border-b border-slate-200">
          <h5 className="font-semibold text-slate-700 text-sm">Struktur File Utama</h5>
        </div>
      ...
      <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm">
        <div className="bg-slate-50 px-4 sm:px-6 py-3.5 border-b border-slate-200 flex items-center gap-4">
           ...
           <h5 className="font-semibold text-slate-700 text-xs sm:text-sm font-mono border-l border-slate-300 pl-4">src/utils/distance.ts</h5>
        </div>
        <div className="bg-slate-900 overflow-hidden">
            <div className="p-5 sm:p-6 text-slate-300 text-[12px] sm:text-[13px] font-mono leading-loose tabular-nums overflow-x-auto">
                <div className="whitespace-pre"><span className="text-slate-500 italic">// Euclidean Distance dari scratch</span></div>
                ...
                <div className="whitespace-pre">    sum += (a[i] - b[i]) ** <span className="text-orange-300">2</span>;</div>
            ...
            <div className="bg-slate-800/80 p-4 sm:p-5 border-t border-slate-700/50 text-xs sm:text-sm text-slate-300 font-light leading-relaxed">
             <strong className="font-semibold text-white">Rumus:</strong> d(p,q) = √Σ(pᵢ - qᵢ)² — mengukur seberapa jauh titik p dari titik q di ruang multi-dimensi.
           </div>
        </div>
      </div>
      ...
      <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm">
        <div className="bg-slate-50 px-4 sm:px-6 py-3.5 border-b border-slate-200 flex items-center gap-4">
           ...
           <h5 className="font-semibold text-slate-700 text-xs sm:text-sm font-mono border-l border-slate-300 pl-4">src/utils/knn.ts</h5>
        </div>
        <div className="bg-slate-900 p-5 sm:p-6 overflow-x-auto">
          <div className="text-[12px] sm:text-[13px] font-mono leading-loose overflow-hidden tabular-nums">
            <div className="text-slate-500 italic whitespace-pre">// KNN Prediksi dengan Majority Voting</div>
            <div className="whitespace-pre"><span className="text-pink-400">export function</span> <span className="text-blue-400">knnPredict</span>(</div>
            <div className="whitespace-pre">  trainX: <span className="text-teal-300">number</span>[][], trainY: <span className="text-teal-300">number</span>[],</div>
            <div className="whitespace-pre">  testPoint: <span className="text-teal-300">number</span>[], k: <span className="text-teal-300">number</span></div>
            <div className="whitespace-pre">): <span className="text-teal-300">number</span> {'{'}</div>
            <div className="whitespace-pre">  <span className="text-slate-500 italic">{'// 1. Hitung jarak ke semua data latih'}</span></div>
            <div className="whitespace-pre">  <span className="text-pink-400">const</span> distances = trainX.<span className="text-blue-400">map</span>((row, i) =&gt; ({'{'}</div>
            <div className="whitespace-pre">    label: trainY[i],</div>
            <div className="whitespace-pre">    distance: <span className="text-blue-400">euclideanDistance</span>(testPoint, row)</div>
            <div className="whitespace-pre">  {'}'}));</div>
            <br />
            <div className="whitespace-pre">  <span className="text-slate-500 italic">{'// 2. Ambil K tetangga terdekat'}</span></div>
            ...
            <br />
            <div className="whitespace-pre">  <span className="text-slate-500 italic">{'// 3. Voting mayoritas'}</span></div>
            <div className="whitespace-pre">  <span className="text-pink-400">const</span> votes: <span className="text-teal-300">Record</span>&lt;<span className="text-teal-300">number</span>, <span className="text-teal-300">number</span>&gt; = {'{}'};</div>
            <div className="whitespace-pre">  neighbors.<span className="text-blue-400">forEach</span>(n =&gt; votes[n.label] = (votes[n.label] || <span className="text-orange-300">0</span>) + <span className="text-orange-300">1</span>);</div>
            <div className="whitespace-pre">  <span className="text-pink-400">return</span> <span className="text-blue-400">parseInt</span>(Object.<span className="text-blue-400">entries</span>(votes).<span className="text-blue-400">sort</span>((a, b) =&gt; b[<span className="text-orange-300">1</span>] - a[<span className="text-orange-300">1</span>])[<span className="text-orange-300">0</span>][<span className="text-orange-300">0</span>]);</div>
            <div className="whitespace-pre">{'}'}</div>
          </div>
        </div>
      </div>
