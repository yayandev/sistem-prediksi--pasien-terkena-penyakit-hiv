...
export default function Dashboard() {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={loadStats}
            className="mt-3 text-sm text-slate-900 font-semibold underline"
          >
            Coba lagi
          </button>
        </div>
  ...
  const s = stats!;
  const totalClass = (Object.values(s.classDistribution) as number[]).reduce(
    (a, b) => a + b,
    0,
  );
  const totalPred = (
    Object.values(s.predictionDistribution) as number[]
  ).reduce((a, b) => a + b, 0);
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Import 200 data seed realistis untuk mulai menggunakan sistem.
            </p>
          </div>
          <button
      ...
      {imported && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              200 data seed sudah tersimpan di Firestore.
            </p>
          </div>
        </div>
      )}
              ...
              <div className="space-y-4">
                {Object.entries(s.classDistribution).map(([label, count]) => {
                  const pct = Math.round(
                    ((count as number) / totalClass) * 100,
                  );
                  return (
                    <div key={label}>
                      ...
                      <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full ${CLASS_COLORS[label] || "bg-slate-400"} rounded-full transition-all duration-500`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
        ...
        <div className="bg-white border-2 border-slate-900">
          ...
          <div className="p-6">
            {totalPred === 0 ? (
              ...
              <div className="space-y-4">
                {Object.entries(s.predictionDistribution).map(
                  ([label, count]) => {
                    const pct = Math.round(
                      ((count as number) / totalPred) * 100,
                    );
                    return (
                      <div key={label}>
                        <div className="flex items-center justify-between mb-1">
